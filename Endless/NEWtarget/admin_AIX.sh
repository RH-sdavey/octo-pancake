...
clear
echo
echo $XT "Admin Info for AIX machines" $GT
echo
...
DATENUMBER="$(date +%j%s)"
DATEFORLOG="$(date)"
mkdir ./results/information/admin_aix/$DATENUMBER
echo "Script: admin_aix	User: $name	Email: $FEMAIL	Folder: $DATENUMBER	MACHINES: $MACHINES	Date: $DATEFORLOG">> ./logfile.dat ;
echo "Script: admin_aix	User: $name	Email: $FEMAIL	Folder: $DATENUMBER	MACHINES: $MACHINES	Date: $DATEFORLOG">> ./requests/presession.dat ;
...
#read funct
#for q in $MACHINES; do for f in $funct;do echo; echo $YT $q $GT;grep -ih -e $funct ./results/information/admin_aix/$DATENUMBER/admin_$q.txt;echo;echo;done;read -p "$YT Press any key to continue... $GT " -n1 -s; done; ./main.sh;
#elif [ $outputchoice == "2" ] 
...
#then zip -e -m ./results/information/admin_aix/$DATENUMBER/OUT-$FEMAIL.zip ./results/information/admin_aix/$DATENUMBER/*.*
#echo
#mutt -a "./results/information/admin_aix/$DATENUMBER/OUT-$FEMAIL.zip" -s "Admin Info for AIX $MACHINES" < /dev/null -- $FEMAIL 2>/dev/null;
echo
...
#then ./main.sh
#else echo "you messed up, taking you back to the start of this menu"; ./admin_aix.sh;
#fi
