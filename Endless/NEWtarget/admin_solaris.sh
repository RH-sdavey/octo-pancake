...
echo "Script: admin_solaris	User: $name	Email: $FEMAIL	Folder: $DATENUMBER	MACHINES: $MACHINES	Date: $DATEFORLOG">> ./logfile.dat ;
echo "Script: admin_solaris	User: $name	Email: $FEMAIL	Folder: $DATENUMBER	MACHINES: $MACHINES	Date: $DATEFORLOG">> ./requests/presession.dat ;
...
#for i in $MACHINES;do ssh -tA -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -q -A ticket_id=9999@sls4root@10.224.64.69 --command sls -c root@$i "'logger -p auth.info "PHC MAINTENANCE - INFO GATHERING FOR CHANGE CREATION" && date&&echo&&echo&&hostname&&echo&&echo&&echo ********IFCONFIG*******&&ifconfig -a&&echo &&echo &&echo **************CONTENTS OF APPLICATIONS FOLDER************** &&echo &&echo &&ls -l /applications&&echo &&echo &&echo **********THIS IS THE UPTIME************ &&echo &&echo &&uptime &&echo &&echo &&echo ***************PROCESSES RUNNING NOW*********** &&echo &&echo &&ps -ef &&echo &&echo &&echo &&echo &&echo *****************OPEN PORT CONNECTIONS************* &&echo &echo &&netstat -a &&echo &&echo &&echo *********CRONTAB INFO******** &&echo &&echo &&crontab -l &&echo  &&echo &&echo ****MOUNT INFO******** &&echo &&echo &&mount &&echo &&echo &&echo ********SHELLS******* &&echo &&cat /etc/shells &&echo &&echo &&echo ******INSTALLED SERVICES******* &&echo &&cat /var/db/var/installed_x_services &&echo &&echo &&echo *******DISKSPACE DETAILS********* &&echo &&echo && df -h /usr /var /tmp /home /dev / &&echo &&echo &&echo ********OSLEVEL DETAILS*******  &&echo &&echo &&uname -a &&echo &&grep TL /var/db/var/installed_x_services|tail -1  &&echo &&echo &&echo ******ALL FUNCTIONS AND LEVEL********* &&echo&&w&&echo&&echo&&pkginfo -l&&echo '""" > ./results/information/admin_solaris/$DATENUMBER/admin_$i.txt ; done ;
clear
echo 
#menu system#
#cat << EOF
...
#read funct
#for q in $MACHINES; do for f in $funct;do echo; echo $YT $q $GT;grep -ih -e $funct ./results/information/admin_solaris/$DATENUMBER/admin_$q.txt;echo;echo;done;read -p "$YT Press any key to continue... $GT " -n1 -s; done; ./main.sh;
...
#then zip -e -m ./results/information/admin_solaris/$DATENUMBER/OUT-$FEMAIL.zip ./results/information/admin_solaris/$DATENUMBER/*.*
#echo
#mutt -a "./results/information/admin_solaris/$DATENUMBER/OUT-$FEMAIL.zip" -s "Admin Info for Solaris $MACHINES" < /dev/null -- $FEMAIL 2>/dev/null;
echo
...
#elif [ $outputchoice == "3" ] 
#then ./main.sh
#else echo "you messed up, taking you back to the start of this menu"; ./admin_solaris.sh;
#fi
