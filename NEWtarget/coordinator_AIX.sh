...
clear
echo
echo $XT "Coordinator Info for AIX machines" $GT
echo
...
DATENUMBER="$(date +%j%s)"
DATEFORLOG="$(date)"
mkdir ./results/information/coord_aix/$DATENUMBER
echo "Script: coord_aix	User: $name	Email: $FEMAIL	Folder: $DATENUMBER	MACHINES: $MACHINES	Date: $DATEFORLOG" >> ./logfile.dat ;
echo "Script: coord_aix	User: $name	Email: $FEMAIL	Folder: $DATENUMBER	MACHINES: $MACHINES	Date: $DATEFORLOG" >> ./requests/presession.dat ;
...
#read funct
#for q in $MACHINES; do for f in $funct;do echo; echo $YT $q $GT;grep -ih -e $funct ./results/information/coord_aix/$DATENUMBER/coordinator_$q.txt;echo;echo;done;read -p "$YT Press any key to continue... $GT " -n1 -s; done; ./main.sh;
...
#then zip -e -m ./results/information/coord_aix/$DATENUMBER/OUT-$FEMAIL.zip ./results/information/coord_aix/$DATENUMBER/*.*
#echo
#mutt -a "./results/information/coord_aix/$DATENUMBER/OUT-$FEMAIL.zip" -s "Coordinator Info for AIX $MACHINES" < /dev/null -- $FEMAIL 2>/dev/null;
#clear
#echo
...
#then ./main.sh
#else echo "you messed up, taking you back to the start of this menu"; ./coordinator_AIX.sh;
#fi
