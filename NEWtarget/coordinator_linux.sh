...
clear
echo
echo $XT "Coordinator Info for Linux machines" $GT
echo
...
DATENUMBER="$(date +%j%s)"
DATEFORLOG="$(date)"
mkdir ./results/information/coord_linux/$DATENUMBER
echo "Script: coord_linux	User: $name	Email: $FEMAIL	Folder: $DATENUMBER	MACHINES: $MACHINES	Date: $DATEFORLOG">> ./logfile.dat ;
echo "Script: coord_linux	User: $name	Email: $FEMAIL	Folder: $DATENUMBER	MACHINES: $MACHINES	Date: $DATEFORLOG">> ./requests/presession.dat ;
...
#for i in $MACHINES;do ssh -tA -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -q -A ticket_id=9999@sls4root@10.224.64.69 --command sls -c root@$i "'logger -p auth.info "PHC MAINTENANCE - INFO GATHERING FOR CHANGE CREATION" && date&&echo&&echo&&hostname&&echo&&echo &&echo **************CONTENTS OF APPLICATIONS FOLDER************** &&echo &&echo &&ls -l /applications&&echo &&echo &&echo **********THIS IS THE UPTIME************ &&echo &&echo &&uptime &&echo &&echo &&echo ***************ALL PROCESSES RUNNING NOW******************* &&echo &&echo &&ps -ef &&echo &&echo &&echo ******INSTALLED SERVICES******* &&echo &&cat /var/db/var/installed_x_services &&echo &&echo &&echo *******DISKSPACE DETAILS********* &&echo &&echo && df -h /boot /usr /var /tmp /home / &&echo &&echo &&echo ********OSLEVEL DETAILS*******  &&echo &&echo &&uname -a &&echo &&echo &&grep ML /var/db/var/installed_x_services|tail -1 ;cat /etc/*release &&echo &&echo &&echo ******ALL FUNCTIONS AND LEVEL********* &&echo&&w&&echo&&echo&& rpm -qa | sort &&echo '""" > ./results/information/coord_linux/$DATENUMBER/coordinator_$i.txt ; done ;
clear
echo 
#menu system#
#cat << EOF
...
#read funct
#for q in $MACHINES; do for f in $funct;do echo; echo $YT $q $GT; grep -ih -e $funct ./results/information/coord_linux/$DATENUMBER/coordinator_$q.txt;echo;echo;done;read -p $YT "Press any key to continue... " $GT -n1 -s; done; ./main.sh;
...
#then zip -e -m ./results/information/coord_linux/$DATENUMBER/OUT-$FEMAIL.zip ./results/information/coord_linux/$DATENUMBER/*.*
#echo
#mutt -a "./results/information/coord_linux/$DATENUMBER/OUT-$FEMAIL.zip" -s "Coordinator Info for LINUX $MACHINES" < /dev/null -- $FEMAIL 2>/dev/null;
echo
...
#then ./main.sh
#else echo "you messed up, taking you back to the start of this menu"; ./coordinator_linux.sh;
#fi
